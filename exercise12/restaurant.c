...
//end-marker for cook thread, should jump out of the loop (what is not working yet)
...
int main(int argc, char* argv[]) {
    ...
    // loop to wait until all guests pthreads are finished (got their meal)
    for (int i = 0; i < number_of_guests; i++) {
        pthread_join(guests[i], NULL);
        overall_time += guests_info[i].waiting_time_ms; 
        //printf("%d guests are back\n", i);
    ...
    //sending a signal to get the cook_threads over the cond_var_wait
    for (int i = 0; i < number_of_cooks; i++) {
    ...
    //didn't manage to finish it in time.
